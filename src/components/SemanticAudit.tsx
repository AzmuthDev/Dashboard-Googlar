...
export function SemanticAudit({
    ...
    const { campaignList, adGroupList, baseFilteredTerms } = useMemo(() => {
        try {
            ...
            const filtered = termsArray.filter((t: CampaignTerm) => 
                t && 
                (!selectedCampaign || t?.campanha === selectedCampaign) && 
    ...
    const handleToggleQC = async (termId: string, level: 1 | 2, currentValue: boolean) => {
        ...
        // Parse do auditor_comment para salvar o nome de quem validou
        const currentTerm = queryClient.getQueryData<CampaignTerm[]>(['campaign-terms', activeCompanyId, activeCompany?.tableName])?.find(t => t.id === termId);
        let metadata: any = {};
        try {
            metadata = JSON.parse(currentTerm?.auditor_comment || '{}');
        } catch (e) {}

        const auditorName = profile?.name || user?.user_metadata?.name || currentUser?.email?.split('@')[0] || 'Usuário';

        if (newValue) {
            metadata[`user_${column}`] = auditorName;
        } else {
            delete metadata[`user_${column}`];
        }

        // Se desmarcar QC1, força desmarcar QC2 também
        const updatePayload: any = { 
            [column]: newValue,
            auditor_comment: JSON.stringify(metadata)
        };
        if (level === 1 && !newValue) {
            updatePayload['triagem2'] = false;
            delete metadata['user_triagem2'];
            updatePayload['auditor_comment'] = JSON.stringify(metadata);
        }
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (_: any, record: CampaignTerm) => {
                ...
                if (isNeg(record)) {
                    return <div className="w-fit px-3 py-1 rounded-full bg-red-500/10 border border-red-500/20 text-red-500 dark:bg-red-500/20 dark:text-red-400 text-[10px] font-bold uppercase tracking-widest">Negativar</div>;
                }
                if (isDuv(record)) {
                    return <div className="w-fit px-3 py-1 rounded-full bg-amber-500/10 border border-amber-500/20 text-amber-500 dark:bg-amber-500/20 dark:text-amber-400 text-[10px] font-bold uppercase tracking-widest">Dúvida</div>;
                }
                if (isSeg(record)) {
                    return <div className="w-fit px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-500 dark:bg-emerald-500/20 dark:text-emerald-400 text-[10px] font-bold uppercase tracking-widest">Segmentar</div>;
                }
                if (isTst(record)) {
                    return <div className="w-fit px-3 py-1 rounded-full bg-orange-500/10 border border-orange-500/20 text-orange-500 dark:bg-orange-500/20 dark:text-orange-400 text-[10px] font-bold uppercase tracking-widest">Teste A/B</div>;
                }
                
                return record.status_granularidade ? (
                    <div className="w-fit px-2 py-0.5 rounded-full bg-zinc-500/10 border border-zinc-500/20 text-zinc-600 dark:bg-zinc-500/20 dark:text-slate-400 text-[10px] font-medium italic">{record.status_granularidade}</div>
                ) : null;
            }
        },
        {
            ...
            render: (_: any, record: CampaignTerm) => {
                ...
                // Extrai o nome do usuário do campo direto ou do JSON em auditor_comment
                let metadata: any = {};
                try {
                    metadata = JSON.parse(record.auditor_comment || '{}');
                } catch(e) {}
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                        <Select
                            ...
                            className="w-[200px] [&_.ant-select-selector]:!bg-white dark:[&_.ant-select-selector]:!bg-slate-900 [&_.ant-select-selector]:!border-slate-300 dark:[&_.ant-select-selector]:!border-slate-700 [&_.ant-select-selection-item]:!text-slate-900 dark:[&_.ant-select-selection-item]:!text-white [&_.ant-select-selection-placeholder]:!text-slate-500 dark:[&_.ant-select-selection-placeholder]:!text-slate-300 [&_.ant-select-arrow]:!text-slate-900 dark:[&_.ant-select-arrow]:!text-white"
                            popupClassName="!bg-white dark:!bg-slate-900 !border !border-slate-200 dark:!border-slate-700 shadow-xl dark:!shadow-[0_10px_30px_rgba(0,0,0,0.8)] [&_.ant-select-item]:!text-slate-700 dark:[&_.ant-select-item]:!text-slate-200 [&_.ant-select-item-option-active]:!bg-slate-100 dark:[&_.ant-select-item-option-active]:!bg-blue-500 [&_.ant-select-item-option-active]:!text-slate-900 dark:[&_.ant-select-item-option-active]:!text-white [&_.ant-select-item-option-selected]:!bg-blue-100 dark:[&_.ant-select-item-option-selected]:!bg-blue-600 [&_.ant-select-item-option-selected]:!text-blue-700 dark:[&_.ant-select-item-option-selected]:!text-white [&_.ant-select-item-option-content]:!text-inherit"
            ...
            {terms.length > 0 ? (
                ...
                <div className="flex flex-col items-center justify-center py-32 bg-muted/20 rounded-3xl border-2 border-dashed border-border text-center">
                    <TrendingUp className="w-12 h-12 text-blue-500 mb-4 opacity-80" />
                    <h2 className="text-xl font-semibold text-foreground">Nenhum termo encontrado</h2>
                    <p className="text-muted-foreground max-w-xs mt-2">
                        Esta empresa ainda não possui termos auditáveis importados.
                    </p>
                </div>
            )}
