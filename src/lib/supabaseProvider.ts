...
/**
 * Operações de Auditoria LSA
 */
export async function fetchAuditoriaLSA(empresaId: string): Promise<any[]> {
    try {
        const { data, error } = await supabase
            .from('auditoria_lsa')
            .select('*')
        ...
        if (error) {
            console.error(`[Supabase] Erro ao buscar Auditoria LSA para a empresa ${empresaId}:`, error);
            return [];
        }

        return data || [];
    } catch (err) {
        console.error(`[Supabase] Exceção ao buscar Auditoria LSA:`, err);
        return [];
    }
}
